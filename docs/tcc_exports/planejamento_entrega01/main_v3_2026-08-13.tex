...
%% Versão: v3 (2026-08-13) — realinhada: heterogeneidade da evidência
%% como problema central; percurso único; serviço auditável.
%% =====================================================================
...
Enchentes e alagamentos são o desastre de maior recorrência no Brasil e atingem
primeiro quem não escolheu onde morar: terreno baixo, drenagem insuficiente e
ocupação precária são condição física e social ao mesmo tempo. Entre 2000 e 2018
a população exposta a inundação cresceu dez vezes acima do previsto, concentrada
nessas áreas~\cite{tellman2021}. A atenção que um território recebe acompanha
essa desigualdade, e o dado também: o registro oficial nasce de quem consegue
reportar, e descreve o atendimento, não a extensão da água. Ausência de registro não é ausência de
evento~\cite{li2022pul,agostini2024}, e é daí que a pesquisa parte, atrás de
positivos e negativos válidos.

Suscetibilidade é a predisposição do terreno a acumular água sob um dado
forçamento de chuva: há enchente quando chega a um ponto mais água do que ele
consegue escoar ou armazenar. Isso se decompõe em três grandezas. \emph{Quanto chega} é a chuva. \emph{Para onde
...
o HAND~\cite{renno2008}: a altura do ponto acima do curso d'água que o drena, isto
...
O plano evolui do artigo anterior por revisão de literatura e testes
sucessivos, e as decisões que mudaram a rota estão fixadas. Variável derivada
do rótulo produz circularidade: a acurácia balanceada cai de 0,8855 para 0,4834
sem elas. Composição estática de imagem orbital não carrega assinatura de evento
pontual, em três tentativas independentes; ela passa a evidência de contexto e
fila de revisão. A unidade de informação é o evento, não o ponto, o
que dissolveu significância vinda de pseudorreplicação. A definição da
classe negativa decide o que o modelo aprende: o ajuste de maior desempenho
próprio é o que menos transfere. A derivação precisa ser única entre regiões, e a
resolução segue o mecanismo. E a relação terreno--inundação não caduca: em 25
anos de janela expansiva o modelo fica na faixa esperada em todos os cortes e
transfere entre classes de relevo.

O que resta é a vastidão de evidência reunida e não integrada. As fontes
vêm de cadeias de derivação, resoluções e mecanismos distintos,
e um mesmo nome de coluna pode designar variáveis não comensuráveis; o efeito não aparece como erro visível, mas como mapa plausível. A etapa
que precede o treino é de integração e curadoria: uma base de conhecimento
geoespacial única, com proveniência, resolução, mecanismo e unidade
de validação declarados por registro.

O objetivo é converter essa base em conhecimento replicável: ajustar o modelo
onde existe observação declarada e aplicá-lo às regiões brasileiras de mesmas
...
\node[aux] (c4) at (58,-37) {Semelhança entre áreas\\ vizinhas; fila de revisão};
...
governam: a camada orbital (vermelho) não entra no escore; o serviço (verde) só
explica depois que os \emph{gates} decidiram.}
\label{fig:pipeline}
\end{figure*}
...
\textbf{Integração e curadoria: o que precede o treino.} Reunir evidência não é o
gargalo; torná-la comparável é. Uma fonte traz terreno de 10\,m e outra de 30\,m,
o que muda a escala em que HAND e TWI são lidos; parte das evidências documentais
não é geocodificável; áreas de interesse declaradas não coincidem com o recorte
urbano que interessa; cenas ópticas chegam com nuvem; milímetros de chuva não
transferem entre climatologias; e há áreas cujo mecanismo dominante é movimento
de massa. Toda fonte passa por uma
redução a tabela de pontos, e cada linha declara procedência, resolução nativa,
unidade de agrupamento, classe de relevo e mecanismo. Entra no treino apenas o
que está dentro da AOI declarada, tem as variáveis computadas na mesma cadeia e
tem grupo de validação identificável; o restante fica como proveniência, com o
motivo do descarte nomeado.

\textbf{Material próprio, material externo.} Já auditados: a cadeia de derivação
de terreno, reproduzida \emph{bit a bit} contra o raster de Recife; o modelo de
Recife, com 278 pontos; o conjunto de Curitiba, com 1.471 unidades; e o motor de
inferência exposto por contrato. Externas são as bases que publicam observação
declarada de não-inundação. Falta a base integrada, a validação prospectiva e a
camada de explicação.

\textbf{Rótulo positivo e hierarquia do negativo.} Os positivos são eventos
oficialmente registrados e geocodificados: Defesa Civil de Recife, SIAC 156 de
Curitiba, Diário Oficial, ANA/SNIRH e Global Flood Database~\cite{tellman2021}. O
...
evento separado na origem; \emph{exclusão qualificada}, do piloto inglês, por
quatro critérios sobre os \emph{Recorded Flood Outlines}; e \emph{ausência de
registro}, que é o que as três regiões brasileiras têm hoje. A unidade
independente é o evento, ou a AOI, nunca o ponto.
...
os três desafios que orientam o plano. Nenhuma fonte cobre todas as variáveis ---
a chuva só existe no piloto inglês e as bases de rótulo por imagem não têm
derivadas direcionais ---, por isso o modelo multirregião roda com quatro
variáveis de terreno. A definição de negativo muda o que o modelo aprende: o
...
o relevo: o contraste de HAND entre classes vai de cerca de 3\,m em planície a
quase 28\,m em serra.
...
limitações. Quando um \emph{gate} não fecha, retorna \texttt{insufficient\_data}
em vez de inventar escore. É sobre esse \emph{payload} já decidido que a camada
de explicação atua --- o padrão de predição seletiva~\cite{mitchell2019}.
...
versão de biblioteca, não de treino, e se resolve reduzindo toda fonte de imagem
a tabela de pontos. Sem divisão de carga entre pessoas, o paralelismo vem da
sobreposição de etapas. Duas limitações já forçaram ajuste de escopo: o treino
sobre \emph{embeddings} foi abandonado após três tentativas, e o HAND das fontes
externas vem de produto global de 30\,m --- limitação declarada, não
equivalência.
...
SGB/CPRM; CHIRPS~\cite{funk2015} e ERA5-Land~\cite{munoz2021}; Sentinel-1/2 e cobertura
...
\textbf{E2 --- Integração e curadoria da evidência (M1).} Reduzir cada fonte a tabela de
pontos com procedência, resolução, unidade de agrupamento, classe de relevo e
mecanismo; geocodificar o recuperável e registrar o descarte do restante.
\emph{Entregável}: base de conhecimento geoespacial única. \emph{Evidência}: contagem
do que entra e do que sai por fonte, com o motivo nomeado.
...
\emph{Entregável}: tabelas de coeficientes e de desempenho por classe.
\emph{Evidência}: EPV verificado antes de cada ajuste, sinais físicos conferidos
depois, nenhum ajuste após olhar o teste.

\textbf{E4 --- \emph{Holdout} temporal (M3).} Teste ainda não realizado e que não
exige aquisição nova: os eventos do piloto inglês cobrem cerca de 158 datas em 25
anos. \emph{Entregável}: desempenho por corte temporal. \emph{Evidência}: cortes
definidos antes da execução.

\textbf{E5 --- Aplicação às regiões brasileiras (M4).} Levar o modelo às três
regiões e caracterizar a distância entre o domínio de treino e cada uma.
\emph{Entregável}: mapa de suscetibilidade por região, com maturidade declarada.
\emph{Evidência}: nenhuma afirmação de acerto onde não há inventário local.

\textbf{E6 --- Serviço e camada de explicação (M5).} Expor o modelo pelo contrato
e testar a explicação sobre respostas reais, inclusive as de recusa.
\emph{Entregável}: cenários com resposta e explicação correspondente.
\emph{Evidência}: nenhuma explicação contradiz o \emph{payload}.
...
o conjunto encolhe, o critério não. Em E4, se o desempenho temporal não superar o
acaso, o resultado é publicado como está. Em E6, se a explicação divergir do
