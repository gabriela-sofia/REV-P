...
%% Autoras: Gabriela Sofia, Bárbara Borges, Nicole Guarnieri, Rafaella Guerra
%% Formatação: IEEEtran conference (padrão do template dos professores)
%% Compilar: pdflatex main && pdflatex main
%% Versão: reconciliação de 2026-08-21 entre o rascunho do Overleaf
%% (planejamento02_2.pdf: autoria em equipe, abertura reescrita, três notas
%% suas entre colchetes) e o main.tex real do REV-P (v12, 2026-08-20,
%% commit 5d6008b), que tinha E3, E4, E5 e E6 fechados que o rascunho do
%% Overleaf ainda descrevia como plano. Ver a mensagem de entrega para a
%% lista completa do que mudou e o que segue sem resposta sua.
%% =====================================================================
...
\author{\IEEEauthorblockN{Gabriela Sofia, Bárbara Borges, Nicole Guarnieri, Rafaella Guerra}
\IEEEauthorblockA{Experiência Criativa: Projeto Transformador II\\
Bacharelado em Ciência da Computação --- PUCPR\\
Turma 8A --- Equipe 8\\
\{\texttt{gabriela.milani, barbara.borges, nicole.guarnieri, rafaella.guerra}\}@pucpr.edu.br}}
...
Enchentes são o desastre de maior recorrência no Brasil e afetam, de
maneira desproporcional, as populações segregadas espacialmente que não
dispõem de margem de escolha habitacional. Relevo que não escoa a água e
relevo que cede sob ela são duas faces da mesma vulnerabilidade física; da
inviabilidade desse relevo nasce a ocupação urbana precária, e dela, a
vulnerabilidade social. Entre 2000 e 2018 a população exposta a inundação
...
enxurrada ou movimento de massa~\cite{ibge2018risco}. A atenção que um
território recebe acompanha essa desigualdade, e o dado também: o registro
oficial nasce de quem consegue reportar e ser atendido --- a ausência não
reflete a falta de chamados, mas a lacuna de alcance do poder público e da
ação governamental no território. A documentação disponível reflete a
assistência prestada, não a extensão real da água. Ausência de registro não
é ausência de evento~\cite{li2022pul,agostini2024}.
...
ponto mais água do que ele consegue escoar ou armazenar. Isso se decompõe em
três grandezas. \emph{Quanto chega} é a chuva. \emph{Para onde converge} é
o índice topográfico de umidade~\cite{beven1979}, que mede quanta encosta
drena para ali. \emph{Quanto precisa subir para ser alcançado} é
...
O plano se apoia na entrega metodológica desenvolvida previamente,
organizando os principais conceitos, ferramentas e rotas viáveis. Variável
derivada do rótulo produz circularidade: acurácia balanceada cai de 0,8855
para 0,4834 sem elas. Composição estática de imagem orbital não carrega
assinatura de evento pontual em três tentativas independentes; vira
evidência de contexto e fila de revisão. O peso do DINOv2 no projeto fica
assim delimitado: um codificador visual congelado --- nunca ajustado aos
dados do projeto ---, usado só para medir similaridade entre áreas e
alimentar essa fila de revisão; as três tentativas de promovê-lo a preditor
fecharam sem sinal, e essa rota está encerrada como candidata a variável. A
unidade de informação é o evento, não o ponto, dissolvendo significância de
pseudorreplicação. A definição da classe negativa decide o que o modelo
aprende: o ajuste de maior desempenho próprio é o que menos transfere. A
derivação é única entre regiões, na mesma resolução para toda a base, mesmo
onde custa detalhe que o modelo não usa; e a relação terreno--inundação não
caduca em 25 anos de janela expansiva, com transferência entre classes de
relevo.

A base física já está consolidada: as fontes, antes em cadeias de
derivação, resoluções e mecanismos distintos --- onde um mesmo nome de
coluna podia designar variáveis não comensuráveis, sem erro visível, só
mapa plausível --- foram reduzidas a uma base de conhecimento geoespacial
única, com proveniência, resolução, mecanismo e unidade de validação
declarados por registro (E2, Seção~\ref{sec:etapas}). Evoluir, daqui em
diante, é ajustar, testar e corrigir o modelo sobre o que já existe, rodada
a rodada --- não reunir mais dado bruto.

O objetivo é aplicar essa base e a metodologia causal já fixada para
decidir, por AOI, se o terreno é suscetível a enchente: ajustar o modelo
onde existe observação declarada e aplicá-lo às regiões brasileiras de
mesmas características físicas e sem esse registro, entregando o resultado
como serviço auditável (Fig.~\ref{fig:pipeline}).
\textbf{Pergunta de pesquisa}: um modelo interpretável de suscetibilidade a
enchentes, treinado com negativo observado, pode ser aplicado de forma
auditável a territórios urbanos vulneráveis sem inventário local de
eventos?
...
% ---- coluna 4
\node[prd] (p1) at (144,-4)  {\textbf{Contrato de inferência}\\ \emph{gates}: geometria, região,\\ modelo, variáveis, domínio};
...
Curitiba, com 1.471 unidades de validação --- grupos por evento, não pontos
individuais: é o mesmo princípio de agrupamento que rege toda a validação do
projeto, declarado adiante nesta seção ---; a tabela única harmonizada das
seis fontes, hoje com 65.070 pontos elegíveis ao \emph{ajuste fluvial} --- o
ajuste que estima suscetibilidade a enchente, e não a outro mecanismo, como o
movimento de massa que também aparece em Petrópolis ---, nenhum deles mais em
cadeia de terreno global --- ou seja, nenhum depende mais de HAND ou TWI de um
produto genérico; os quatro já foram rederivados pela mesma cadeia D-infinity
própria do projeto, o que torna a variável comparável entre fontes; e o motor
de inferência exposto por contrato. Externas:
...
99,99\% de cobertura na base harmonizada, de produto único. Mas ela é medida em
células de $\sim$11\,km enquanto o modelo compara pontos dentro do mesmo
evento: nas seis fontes a chuva não discrimina nessa escala, e por isso entra
como forçamento declarado, não como propriedade do lugar que o modelo aprende
a distinguir. Quantas dessas variáveis entram em cada ajuste é decisão de E3,
não deste plano. A definição de negativo muda o
...
camada de explicação atua: o padrão de predição seletiva~\cite{mitchell2019}.
Nessa camada, o DINOv2 entra só como evidência anexada à explicação --- a
similaridade entre a área consultada e áreas já avaliadas ilustra a resposta
para quem revisa, mas não é lida pelo contrato: nenhum \emph{gate} a consulta,
e ela nunca desloca o escore (Fig.~\ref{fig:pipeline}, Limite 1 e Limite 2). São
...
\textbf{Infraestrutura, carga e ajustes de escopo.} O ajuste de Firth leva
0,056\,s e o de um modelo aditivo explicável 25,3\,s no maior conjunto, sem
exigir GPU. O gargalo medido é de carregamento de arquivo e de
...
isolando a incompatibilidade de biblioteca da linha causal. Duas limitações já forçaram ajuste de escopo: o treino
...
relatados. Em E6, a camada de explicação é gerador por regras sobre o \emph{payload}
já decidido, o que impede divergência por construção. Nenhuma altera a pergunta de
pesquisa.
...
arte já está em curso e permanece ativo até a redação. As etapas se sobrepõem
no tempo em vez de correr estritamente em sequência; foram identificadas de
antemão sem que a ordem de conclusão seja obrigatória --- o que muda de
posição é a etapa, nunca o critério de evidência
