...
Indicadores disponíveis com dados GIS locais: distância ao rio (GeoJSON de hidrografia disponível para as três regiões), densidade viária (Recife apenas, via segmentos municipais de vias). Uso do solo é BLOCKED (fiona não instalado; shapefile FBDS existe apenas para Petrópolis). Densidade populacional é BLOCKED (sem dados censitários encontrados).
