...
AS DUAS FEATURES, e como cada uma e definida:
  ...
  rain_decay_index_api    Antecedent Precipitation Index. Soma ponderada da
                          ...
                          feature signifique a mesma coisa nas duas regioes.
...
Uso:
    python scripts/externo/ft_uk04_features_chuva.py
"""
...
def main() -> int:
    ...
    print(f"AMOSTRA n={len(df):,} pos={int((df.label==1).sum()):,} "
          f"neg={int((df.label==0).sum()):,}")

    # --- pareamento temporal do negativo ---
    datas_por_wc = (df[df.label == 1]
                    .groupby("worldcover")["start_date"]
                    .apply(lambda s: s.dropna().dt.date.tolist())
                    .to_dict())
    todas = df.loc[df.label == 1, "start_date"].dropna().dt.date.tolist()
    ...
    for flavor in ("rnl", "sat"):
        ...
        print(f"[{flavor}] rain_max_24h  pos={np.nanmedian(rmax[df.label==1]):.2f} "
              f"neg={np.nanmedian(rmax[df.label==0]):.2f}")
        print(f"[{flavor}] api           pos={np.nanmedian(api[df.label==1]):.2f} "
              f"neg={np.nanmedian(api[df.label==0]):.2f}")
