...
CADEIA:
  aquisicao      ext_uk01 (outlines) -> ext_uk04 (flood zones)
                 aq_master (DEM/HAND/WorldCover/GSW) -> aq_chirps3 (chuva)
...
SUSC = REPO / "scripts" / "suscetibilidade"

# (grupo, script, args, artefato que comprova conclusao, precisa_de_rede)
CADEIA = [
    ("aquisicao", AQUI / "ext_uk01_download_recorded_flood_outlines.py", [],
     RUNS / "ext-uk-01-aquisicao" / "inventario.json", True),
    ("aquisicao", AQUI / "ext_uk04_flood_zones_aoi.py", [],
     RUNS / "ext-uk-04-flood-zones" / "resumo.json", True),
    ("aquisicao", AQUI / "aq_master.py", ["--baixar"],
     RUNS / "aq-features" / "download.json", True),
    ("aquisicao", AQUI / "aq_chirps3_aoi.py", ["--baixar"],
     RUNS / "aq-chirps3" / "aquisicao.json", True),
    ("aquisicao", AQUI / "n1_run_all.py", [],
     RUNS / "n1-relatorio-consolidado.md", True),
    ("aquisicao", AQUI / "aq_rs01_features.py", ["--baixar"],
     RUNS / "aq-rs-features" / "PROVENIENCIA.md", True),

    ("organizacao", AQUI / "ext_uk07_converter_geoparquet.py", [],
     RUNS / "geostore" / "outlines_elegiveis.parquet", False),

    ("analise", AQUI / "ext_uk02_selecionar_area_piloto.py", [],
     RUNS / "ext-uk-02-area-piloto" / "selecao.json", False),
    ("analise", AQUI / "ext_uk03_dimensionar_aoi.py", [],
     RUNS / "ext-uk-03-aoi" / "dimensionamento.json", False),
    ("analise", AQUI / "ext_uk05_contabilidade_negativo.py", [],
     RUNS / "ext-uk-05-contabilidade" / "contabilidade.json", False),
    ("analise", AQUI / "ft_uk01_preparar_rasters_aoi.py", [],
     RUNS / "ft-uk-01-rasters" / "resumo.json", False),
    ("analise", AQUI / "ft_uk02_mascaras_decisao.py", [],
     RUNS / "ft-uk-02-mascaras" / "resumo.json", False),
    ("analise", AQUI / "ft_uk03_amostrar_e_extrair.py", [],
     RUNS / "ft-uk-03-amostra" / "resumo.json", False),
    ("analise", AQUI / "ft_uk04_features_chuva.py", [],
     RUNS / "ft-uk-04-chuva" / "resumo.json", False),
    ("analise", AQUI / "n1f_reduzir_a_pontos.py", [],
     RUNS / "n1f-pontos" / "resumo.json", False),

    ("modelo", SUSC / "ds01_montar_multirregiao.py", [],
     RUNS / "ds-01-multirregiao" / "resumo.json", False),
    ("modelo", SUSC / "mod_uk00_smoke_topografico.py", [],
     RUNS / "mod-uk-00-smoke" / "resumo.json", False),
    ("modelo", SUSC / "mod_uk01_firth_agrupado.py",
     ["--confirmo-pre-requisitos"],
     RUNS / "mod-uk-01-firth" / "resumo_rnl.json", False),
]
