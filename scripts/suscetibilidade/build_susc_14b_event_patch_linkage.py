"""SUSC-14B FASE 6 - event-patch linkage from official reference matches."""
...
LINKAGE = ROOT / "datasets" / "suscetibilidade" / "susc_14b_event_patch_linkage_v1.csv"
SUMMARY = ROOT / "outputs_public" / "suscetibilidade" / "SUSC_14B_event_patch_linkage_summary.csv"
LIMITS = ROOT / "outputs_public" / "suscetibilidade" / "SUSC_14B_event_patch_linkage_limitations.json"
GEOJSON = ROOT / "outputs_public" / "suscetibilidade" / "SUSC_14B_event_patch_linkage_geojson.geojson"
...
def _mk(row, patch_id, relation, distance, score, score_class, interp, limit):
    confidence = "moderate" if relation in {
        "moderate_geocoded_point_inside_patch",
    ...
    can_eval = (
        ...
        and relation in {"moderate_geocoded_point_inside_patch", "moderate_street_segment_intersects_patch", "moderate_street_segment_near_patch"}
...
def main() -> int:
    print("=" * 60)
    print("SUSC-14B Event-Patch Linkage")
    print("=" * 60)
    ...
    for row in read_csv(MATCHED) if MATCHED.exists() else []:
        ...
        if row.get("georeferencing_status") == "official_neighborhood_only":
            links.append(_mk(row, "REGION_LEVEL_NO_PATCH_RESOLUTION", "same_neighborhood_context", None, "", "",
        ...
        for pid, pbb, cen in patches.get(region, []):
            ...
            if pt:
                ...
                if inside:
                    relation = "moderate_geocoded_point_inside_patch" if row.get("matched_feature_type") == "address_point" else "moderate_street_segment_intersects_patch"
                elif near:
    ...
    for idx, row in enumerate(links):
        row["linkage_id"] = f"S14BLINK_{idx:06d}"
    ...
    for link in links:
        ...
        for pid, _bb, cen in patches.get(link["region"], []):
            if pid == link["patch_id"]:
                ...
                break
    write_json(GEOJSON, {"type": "FeatureCollection", "name": "susc_14b_event_patch_linkage", "review_only": True,
                         "can_be_ground_truth": False, "can_be_used_as_ground_truth": False, "allowed_for_training": False,
                         "features": features})
    write_json(LIMITS, {"artifact": "SUSC-14B official reference event-patch linkage",
