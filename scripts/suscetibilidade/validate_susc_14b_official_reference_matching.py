"""SUSC-14B official reference matching validator."""
...
REQUIRED = [
    MAN / "susc_14b_official_spatial_reference_registry_v1.csv",
    MAN / "susc_14b_official_spatial_reference_download_manifest_v1.csv",
    DAT / "official_spatial_references_susc14b",
    DAT / "susc_14b_official_spatial_reference_features_v1.csv",
    DAT / "susc_14b_matched_official_occurrences_v1.csv",
    DAT / "susc_14b_event_patch_linkage_v1.csv",
    OUT / "SUSC_14B_official_spatial_reference_discovery_report.md",
    OUT / "SUSC_14B_official_spatial_reference_acquisition_report.md",
    OUT / "SUSC_14B_official_spatial_reference_parse_audit.csv",
    OUT / "SUSC_14B_address_normalization_audit.csv",
    OUT / "SUSC_14B_occurrence_reference_match_audit.csv",
    OUT / "SUSC_14B_event_patch_linkage_summary.csv",
    OUT / "SUSC_14B_event_patch_linkage_limitations.json",
    OUT / "SUSC_14B_event_patch_linkage_geojson.geojson",
    OUT / "SUSC_14B_observational_readiness.csv",
    OUT / "SUSC_14B_observational_readiness.md",
    OUT / "SUSC_14B_score_v6_event_diagnostics.csv",
    OUT / "SUSC_14B_score_v6_event_diagnostics_summary.json",
    OUT / "SUSC_14B_official_reference_matching_report.md",
]
...
def main() -> int:
    print("=" * 60)
    print("SUSC-14B Official Reference Matching Validation")
    print("=" * 60)
    errors: list[str] = []
    ...
    tables = {
        "registry": read_csv(MAN / "susc_14b_official_spatial_reference_registry_v1.csv"),
        "downloads": read_csv(MAN / "susc_14b_official_spatial_reference_download_manifest_v1.csv"),
        "features": read_csv(DAT / "susc_14b_official_spatial_reference_features_v1.csv"),
        "matched": read_csv(DAT / "susc_14b_matched_official_occurrences_v1.csv"),
        "linkage": read_csv(DAT / "susc_14b_event_patch_linkage_v1.csv"),
        "readiness": read_csv(OUT / "SUSC_14B_observational_readiness.csv"),
        "diag": read_csv(OUT / "SUSC_14B_score_v6_event_diagnostics.csv"),
        "match_audit": read_csv(OUT / "SUSC_14B_occurrence_reference_match_audit.csv"),
    }

    print("[2/12] Governance flags are closed...")
    for label, rows in tables.items():
        errors.extend(_governance(rows, label))
    for path in (OUT / "SUSC_14B_event_patch_linkage_limitations.json", OUT / "SUSC_14B_event_patch_linkage_geojson.geojson", OUT / "SUSC_14B_score_v6_event_diagnostics_summary.json"):
        obj = read_json(path)
    ...
    print("[4/12] Street segment never becomes strong...")
    for idx, row in enumerate(tables["linkage"], start=2):
    ...
    print("[5/12] Observational evaluation requires moderate official match...")
    for idx, row in enumerate(tables["linkage"], start=2):
        if row.get("can_be_used_for_observational_evaluation") == "true":
    ...
    if read_json(OUT / "SUSC_14B_score_v6_event_diagnostics_summary.json").get("score_v7_created") is not False:
    ...
    print("[9/12] Report contains mandatory statement...")
    report = (OUT / "SUSC_14B_official_reference_matching_report.md").read_text(encoding="utf-8")
    if MANDATORY not in report:
        errors.append("report missing mandatory statement")
    else:
        print("  OK")

    print("[10/12] Matched occurrences have required columns...")
    required_match = {"match_score", "name_similarity", "neighborhood_match", "unique_candidate", "geometry_available", "match_confidence"}
    if required_match - set(tables["matched"][0]):
        errors.append(f"matched missing columns: {sorted(required_match - set(tables['matched'][0]))}")
    else:
        print("  OK")

    print("[11/12] Raw directory is gitignored...")
    if not (DAT / "official_spatial_references_susc14b" / ".gitignore").exists():
    ...
    if not [e for e in errors if "readiness missing" in e]:
        print("  OK")
    ...
    if errors:
        ...
        for error in errors:
            print(f"  ERROR: {error}")
        return 1
    print("PASSED: All SUSC-14B validations passed")
    return 0
