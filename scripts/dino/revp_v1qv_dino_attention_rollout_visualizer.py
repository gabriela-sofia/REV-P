...
DOC = _p("REVP_V1QV_DOC", DOCS / "revp_v1qv_dino_attention_rollout_visualizer.md")

MAN_FIELDS = [
    "rollout_id", "smoke_id", "patch_id", "region", "relative_path",
...
def run() -> None:
    ...
    if gate != "EXECUTE":
        ...
        for i, r in enumerate(rows[:max_n], 1):
            manifest.append({
                ...
                "patch_id": r.get("patch_id", ""), "region": normalize_region(r.get("region", "")),
                ...
                "status": final, "blocked_reason": gate.lower(), "notes": "",
            })
    ...
    for i, r in enumerate(rows[:max_n], 1):
        ...
        row: dict[str, Any] = {
            ...
            "region": region, "relative_path": rel, "output_png_relative_path": "",
        ...
        try:
            ...
            out_rel = f"{region.lower() if region != 'PET' else 'petropolis'}/{patch.lower()}__attention_rollout.png"
