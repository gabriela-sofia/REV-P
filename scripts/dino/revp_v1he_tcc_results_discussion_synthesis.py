...
class Evidence:
    ...
    def __init__(self) -> None:
        ...
        # v1hb
        self.hb_manifest = _read_csv(V1HB_DIR / "review_gate_execution_manifest_v1hb.csv")
        ...
        self.hd_examples = _read_csv(V1HD_DIR / "review_gate_visual_examples_for_tcc_v1hd.csv")
        ...
        self.n_review_candidates = self.gz_summary.get("review_gate_candidates", 47)
...
## 4.5 Candidatos de revisão supervisora
...
## 4.6 revisão visual programática (v1hc/v1hd)
...
*Números reais extraídos de artefatos computados. Revisão supervisora obrigatória antes do Overleaf.*
"""
...
> foi criado. Revisão supervisora obrigatória antes da inserção no Overleaf.
...
**f) Evidência visual programática**
A revisão visual programática de {ev.n_visual_computed}/{ev.n_review_candidates} candidatos
...
- **Validação contra ground truth**: Não existe referência de campo (ground truth)
  neste estudo. A revisão visual programática não é validação supervisionada — é
...
estrutural para comparação visual entre regiões. A revisão visual programática
...
- A revisão visual programática parcialmente compensa essa lacuna ao fornecer
...
4. **A revisão supervisora é metodológica**: os candidatos de revisão foram selecionados
...
4. Revisão visual programática por estatísticas de imagem — não substitui inspeção
...
FIGURE_CAPTIONS = [
    ...
    },
    {
        ...
        "caption_pt": (
            "Distribuição dos 47 candidatos de revisão supervisora por categoria "
        ...
        "claim_scope": "Seleção exploratória de candidatos para revisão supervisora",
        "limitation_note": (
...
TABLE_CAPTIONS = [
    ...
    },
    {
        ...
        "caption_pt": (
            "Resumo dos 47 candidatos de revisão supervisora por categoria de seleção. "
            "Seleção baseada em critérios estruturais de embedding e cobertura GIS — "
            "sem atribuição automática de classe ou risco."
        ),
        "tcc_section": "4. Resultados | 4.4",
        "claim_scope": "Candidatos para revisão supervisora por categoria exploratória",
...
CLAIM_MATRIX = [
    ...
    },
    {
        ...
        "evidence_artifact": "review_gate_execution_manifest_v1hb.csv; visual_review_preview_manifest_v1hc.csv",
    ...
    {
        "claim_allowed": "Revisão visual programática por estatísticas de imagem para 47/47 candidatos",
        "result_supporting_it": "NDVI médio, reflexão e heterogeneidade computados de TIFs Sentinel",
        "evidence_artifact": "review_gate_visual_annotation_v1hd.csv; review_gate_visual_summary_v1hd.json",
        "figure_or_table": "v1hc figures (local_runs/dino_embeddings/v1hc/figures/)",
        "limitation": "Revisão programática, não humana direta; interpretação estatística",
...
### 4.4 Candidatos de revisão supervisora
...
### 4.5 revisão visual programática
...
**C — protocolo de revisão supervisora:**
- `docs/metodologia_cientifica/review_gate_protocol.md`
