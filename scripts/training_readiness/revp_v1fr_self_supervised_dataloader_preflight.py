...
def qa_rows(script_text: str, outputs_created: bool, dl_rows: list[dict[str, str]], summary: dict) -> list[dict[str, str]]:
    ...
    add("canonical writes absent", True, "Script writes only v1fr outputs, report, script and CLAUDE.md delta.")
...
def claude_delta() -> str:
    return """

---

## Delta v1fr — Self-Supervised Dataloader Preflight

- v1fr created a review-only pre-dataloader scaffold from v1fp/v1fq.
- No training, labels, targets, model metrics, raster pixel reads, gates, CRS promotion, patch-bound promotion or preflight-ready promotion.
- Future use is limited to reviewer-approved self-supervised representation/embedding planning with grouped splits by region/source/asset lineage.
- Final state remains review-only: `patch_bound_validated=0/59`, `preflight_ready=0/59`, gates blocked.
"""
...
def main() -> None:
    ...
    if claude.exists():
        ...
        if "Delta v1fr" not in text:
            claude.write_text(text + claude_delta(), encoding="utf-8")
