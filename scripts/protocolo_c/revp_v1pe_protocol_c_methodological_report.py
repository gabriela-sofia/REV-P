"""REV-P v1pe — Protocol C consolidated methodological report.

Generates a Markdown report structured for TCC writing guidance.
Reads existing summaries — does not invent or inflate findings.
"""
...
OUT_REPORT = _p("REVP_V1PE_OUT_REPORT", DOCS / "revp_v1pe_protocol_c_methodological_report.md")
IN_V1OT = _p("REVP_V1PE_IN_V1OT", DATASETS / "recife_scene_date_recovery_final_scientific_summary_v1ot.csv")
IN_V1PA = _p("REVP_V1PE_IN_V1PA", DATASETS / "recife_protocol_c_observed_evidence_scientific_summary_v1pa.csv")


def _m_ot(metric: str) -> str:
    return load_metric_from_summary(IN_V1OT, metric)


def _m_pa(metric: str) -> str:
    return load_metric_from_summary(IN_V1PA, metric)


def build_report() -> str:
    patches = _m_ot("total_patch_alias_candidates_evaluated")
    product_dates = _m_ot("product_dates_confirmed_real")
    sources = _m_pa("sources_scanned")
    candidates = _m_pa("source_candidates_found")
    contextual = _m_pa("contextual_only_evidence")
    blocked = _m_pa("blocked_insufficient_evidence")
    linkages = _m_pa("event_patch_linkages_total")
    temporal_confirmed = _m_pa("temporal_linkages_confirmed")
    c1 = _m_pa("c1_contextual")
    c2 = _m_pa("c2_review_only")
    dino = _m_pa("dino_review_queue")

    return f"""# Protocolo C - Relatorio Metodologico Consolidado
...
- Patches avaliados: {patches}
- Datas de produto confirmadas: {product_dates}
...
Em {patches} tentativas, nenhuma cadeia completa foi confirmada.
...
- Fontes escaneadas: {sources}
- Candidatos permitidos: {candidates}
- Evidencia contextual: {contextual}
- Evidencia bloqueada: {blocked}
- Vinculos evento-patch: {linkages}
- Vinculos temporais confirmados: {temporal_confirmed}
...
Os {linkages} vinculos evento-patch gerados sao todos contextuais ou
...
|-------|----------|-------------|
| C1    | {c1}     | Contextual apenas — evidencia territorial documentada |
| C2    | {c2}     | Review-only — representacao DINO sem label |
...
visual. A fila DINO review-only contem {dino} entradas. Nenhum target,
...
recuperacao temporal avaliou {patches} patches Sentinel unicos via cadeia de
proveniencia patch-asset-produto-aquisicao. A camada observacional escaneou
{sources} arquivos do repositorio, identificando {candidates} candidatos a fontes
...
{patches} tentativas de resolucao de data produziu cadeia de proveniencia
confirmada (product_dates_confirmed_real=0). A camada observacional classificou
{c1} candidatos como C1 (contextual) e {c2} como C2 (review-only), sem alcance
...
def run() -> None:
    report = build_report()
    emit_doc(OUT_REPORT, report)
    print(f"[v1pe] Methodological report generated: {OUT_REPORT}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="v1pe Protocol C methodological report")
    parser.parse_args()
    run()
