...
# Original: local_runs/recife_modelo_v12_extracao_final/build_v12_dataset.py
...
common_cols = ["point_id", "region", "label", "lat", "lon", "event_date", "neighborhood",
               "negative_source_type", "elevation_m", "slope_deg", "hand_m_dinf", "twi_dinf",
               "rain_max_24h_chirps", "rain_decay_index_api_chirps", "mapbiomas_class_2023",
               "rain_data_source"]
new_pos_a_slice = new_pos_a[common_cols].copy()
new_pos_a_slice["dataset_role_source"] = "LEADA_V12_DIARIO_OFICIAL_DECRETO_35669_28MAI2022_BAIRRO_LEVEL"
new_pos_c_slice = new_pos_c[common_cols].copy()
new_pos_c_slice["dataset_role_source"] = "LEADC_V12_GLOBAL_FLOOD_DATABASE_DFO3291_EVENT_EXTENT_CENTROID"

v9_slice = v9[common_cols + ["dataset_role_source"]].copy()

combined = pd.concat([v9_slice, new_pos_a_slice, new_pos_c_slice], ignore_index=True, sort=False)
print("combined n =", len(combined), f"({(combined.label==1).sum()} pos / {(combined.label==0).sum()} neg)")
...
sar_slice = sar[["point_id", "delta_vv_db", "delta_vh_db", "same_scene_ambiguous",
                 "before_gap_days", "after_gap_days"]].rename(
    columns={"delta_vv_db": "sar_delta_vv_db_INFORMATIONAL_NOT_MODELED",
             "delta_vh_db": "sar_delta_vh_db_INFORMATIONAL_NOT_MODELED"})
combined = combined.merge(sar_slice, on="point_id", how="left")

combined.to_csv(f"{V12}/dataset_v12_final.csv", index=False)
with open(f"{V12}/v12_orthogonalization_stats.json", "w") as f:
