...
from rain_features_on_demand import rain_features_for_query

IN_V12 = Path(__file__).resolve().parents[3] / "linha_causal" / "susc_20a_aquisicao_eventos_reais_recife" / "dataset" / "dataset_eventos_features_v12_final.csv"
...
def run(sample_n: int = 30) -> None:
    ...
    for r in target_rows[:sample_n]:
        ...
        if d1 > 0.5 or d2 > 0.5:
            mismatches.append({"point_id": r["point_id"], "event_date": r["event_date"],
