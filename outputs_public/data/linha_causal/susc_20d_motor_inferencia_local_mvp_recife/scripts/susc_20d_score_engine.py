...
IN_V12 = SUSC20A / "dataset" / "dataset_eventos_features_v12_final.csv"
IN_PUBLISHED_COEFS = SUSC20C / "results" / "primaria_v12_firth_multivariate_coefs.csv"
IN_PUBLISHED_BOOT = SUSC20C / "results" / "primaria_v12_bootstrap_coefs.csv"
...
def load_dino_evidence_index() -> tuple[dict[str, tuple[float, float, float, float]], dict[str, list[float]]]:
    ...
    emb_csv_raw = os.environ.get(
        ...
        str(next(_p for _p in (Path(__file__).resolve(), *Path(__file__).resolve().parents) if (_p / ".git").is_dir() and (_p / "environment.yml").is_file()) / "datasets" / "dino_recife_sedec_all_embeddings_v1r3.csv"))
...
def score_point(x_row: dict[str, float], scaler: StandardScaler, beta_point: np.ndarray,
    ...
    for i, feat in enumerate(FEATURE_COLS, start=1):
        contributions.append({"feature": feat, "contribution": float(beta_point[i] * xs[i - 1])})
