...
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "linha_causal" / "susc_20f_pipeline_geoprocessamento_sob_demanda_recife" / "scripts"))
from on_demand_feature_engine import compute_features_on_demand, is_within_terrain_coverage  # noqa: E402

from region_registry import REGIONS, find_region_for_bbox
...
def gate_region_supported(geom) -> tuple[bool, str, str | None]:
    ...
    region = find_region_for_bbox(lon_min, lat_min, lon_max, lat_max)
    if region is None:
        return False, "regiao_fora_da_cobertura_conhecida (recife/curitiba/petropolis)", None
    return True, "", region


def gate_model_valid_for_region(region: str) -> tuple[bool, str]:
    info = REGIONS[region]
    if info.model_version is None:
...
def evaluate_gates(geojson: dict, crs: str, known_points: list[dict], end_date: date | None = None) -> dict:
    ...
    if not ok:
        return {"status": "insufficient_data", "region_maturity": "insufficient",
                "region": None, "matched_points": [], "computed_features": None, "blockers": [reason]}
    ...
    if not ok or matched_region is None:
        return {"status": "region_not_supported", "region_maturity": "insufficient",
                "region": None, "matched_points": [], "computed_features": None, "blockers": [reason]}
    region: str = matched_region

    region_info = REGIONS[region]

    ok, reason = gate_model_valid_for_region(region)
    if not ok:
        blockers.append(reason)
        return {"status": "insufficient_data", "region_maturity": region_info.region_maturity,
                "region": region, "matched_points": [], "computed_features": None, "blockers": blockers}

    ok, reason, matched, computed = gate_physical_features_available(region, geom, known_points, end_date)
    if not ok:
        blockers.append(reason)
        return {"status": "insufficient_data", "region_maturity": region_info.region_maturity,
                "region": region, "matched_points": [], "computed_features": None, "blockers": blockers}
    ...
    return {"status": "ok", "region_maturity": region_info.region_maturity,
            "region": region, "matched_points": matched, "computed_features": computed, "blockers": []}
