...
_SUSC20D_SCRIPTS = Path(__file__).resolve().parents[3] / "linha_causal" / "susc_20d_motor_inferencia_local_mvp_recife" / "scripts"
if str(_SUSC20D_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(_SUSC20D_SCRIPTS))

import numpy as np  # noqa: E402

from susc_20d_score_engine import (  # noqa: E402
...
class RecifeEngine:
    ...
    def __init__(self) -> None:
        ...
        self.known_points = [
            {"point_id": j["point_id"], "lat": j["lat"], "lon": j["lon"],
             "label": j["label"], **{f: j[f] for f in FEATURE_COLS}}
            for j in self.kept
        ]
