...
RESULTS = ROOT / "outputs_public/data/linha_causal/susc_20k_siac156_curitiba_flood_candidates/results"
...
def hgb_temporal_holdout(df_train: pd.DataFrame, df_test: pd.DataFrame, feature_cols: list[str],
    ...
    return {"n_train": int(len(dtr)), "n_test": int(len(dte)), "monotonic": monotonic,
...
def monotonic_pd_check(clf: HistGradientBoostingClassifier, df_ref: pd.DataFrame,
    ...
    for i, feat in enumerate(feature_cols):
        ...
        rows.append({"feature": feat, "expected_sign": EXPECTED_SIGN[feat],
                     "pd_at_grid_min": round(float(avg[0]), 4), "pd_at_grid_max": round(float(avg[-1]), 4),
                     "monotonic": monotonic, "n_sign_changes_in_slope": n_sign_changes,
                     "direction": direction,
                     "direction_matches_expected": bool(
                         (EXPECTED_SIGN[feat] > 0 and direction == "crescente") or
                         (EXPECTED_SIGN[feat] < 0 and direction == "decrescente"))})
    return pd.DataFrame(rows)
...
def main() -> int:
    ...
    grid_mono.to_csv(out / "v21a_hyperparam_grid_monotonic.csv", index=False)
    grid_mono_summary = {
        "n_combos": int(len(grid_mono)), "auc_min": round(float(grid_mono["holdout_auc_2026"].min()), 4),
        "auc_median": round(float(grid_mono["holdout_auc_2026"].median()), 4),
        "auc_max": round(float(grid_mono["holdout_auc_2026"].max()), 4),
        "pct_above_linear_baseline": round(float((grid_mono["holdout_auc_2026"] > REFERENCE_LINEAR_HOLDOUT_2026).mean() * 100), 1),
        "pct_above_gam_best": round(float((grid_mono["holdout_auc_2026"] > REFERENCE_GAM_BEST_V20Y).mean() * 100), 1),
        "pct_at_or_above_gbm_unrestricted": round(float((grid_mono["holdout_auc_2026"] >= REFERENCE_GBM_UNRESTRICTED_V20U).mean() * 100), 1),
    }
    ...
    pd_check.to_csv(out / "v21a_monotonic_pd_check.csv", index=False)
    ...
    resumo = {
        ...
        "monotonic_holdout_auc_default": r_mono["holdout_auc_2026"],
        "unrestricted_holdout_auc_same_hparams": r_unrest["holdout_auc_2026"],
        "custo_de_impor_monotonicidade_default": round(r_unrest["holdout_auc_2026"] - r_mono["holdout_auc_2026"], 4),
        ...
        "todas_5_curvas_pd_batem_direcao_esperada": bool(pd_check["direction_matches_expected"].all()),
    ...
    (out / "v21a_all_reports.json").write_text(
