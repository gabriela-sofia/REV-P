...
DATASET = (ROOT / "outputs_public/data/linha_causal/susc_20k_siac156_curitiba_flood_candidates/"
                  "registries/v20l_dataset_curitiba_features_v1.csv")
...
def qa_reproducao_recife_v12() -> dict:
    ...
    rec = ROOT / "outputs_public/data/linha_causal/susc_20a_aquisicao_eventos_reais_recife/dataset/dataset_eventos_features_v12_final.csv"
    pub = ROOT / "outputs_public/data/linha_causal/susc_20c_modelagem_validacao_estatistica_rigorosa_recife/results/primaria_v12_firth_multivariate_coefs.csv"
    ...
    cmp = esperado.merge(got, on="feature", suffixes=("_publicado", "_reproduzido"))
    diffs = {c: float(np.abs(cmp[f"{c}_publicado"] - cmp[f"{c}_reproduzido"]).max())
             for c in ["coef_standardized", "p_value", "ci_low_95", "ci_high_95"]}
    return {"status": "REPRODUZIDO" if max(diffs.values()) == 0.0 else "DIVERGENTE",
            "n_features": len(cmp), "max_abs_diff": diffs}
...
def main(argv=None) -> int:
    ...
    print(f"\ndataset: {len(linhas)} linhas -> {len(unidades)} unidades de observacao "
          f"({int((unidades.label == 1).sum())} pos / {int((unidades.label == 0).sum())} neg)")

    reports = {"qa_reproducao_recife_v12": qa,
               "unidade_de_analise": "(lat, lon, event_date) -- linhas pseudo-replicadas excluidas",
