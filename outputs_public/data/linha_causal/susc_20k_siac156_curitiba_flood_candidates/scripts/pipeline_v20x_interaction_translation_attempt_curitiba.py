...
def add_threshold_terms(df: pd.DataFrame) -> pd.DataFrame:
    d = df.copy()
    for feat, (name, cut, direction) in THRESHOLDS.items():
        d[name] = (d[feat] < cut).astype(float) if direction == "lt" else (d[feat] > cut).astype(float)
    return d
...
def run_all_configs(df_train: pd.DataFrame, df_test: pd.DataFrame) -> pd.DataFrame:
    ...
    for name, (tr, te, feats) in configs.items():
        ...
        rows.append({"config": name, "n_features": len(feats), "holdout_auc_2026": auc})
...
def main() -> int:
    ...
    df.to_csv(out / "v20x_interaction_translation_attempts.csv", index=False)

    resumo = {
        "reference_baseline_linear_5feat": 0.5246,
        "reference_gbm_v20u": 0.5888,
        "melhor_config_interpretavel": df.loc[df["holdout_auc_2026"].idxmax(), "config"],
        "melhor_auc_interpretavel": float(df["holdout_auc_2026"].max()),
        "recuperou_o_sinal_do_gbm": bool(df["holdout_auc_2026"].max() >= 0.57),
    }
    print(json.dumps(resumo, indent=2, ensure_ascii=False))
    (out / "v20x_all_reports.json").write_text(
