...
from pipeline_v20y_gam_spline_curitiba import DEFAULT_C, DEFAULT_DEGREE, DEFAULT_N_KNOTS  # noqa: E402
...
def gam_tensor_holdout_auc(df_train: pd.DataFrame, df_test: pd.DataFrame,
    ...
    return {"n_train": int(len(dtr)), "n_test": int(len(dte)),
            "n_interaction_pairs": len(interaction_pairs), "n_features_total": int(Xtr.shape[1]),
...
def main() -> int:
    ...
    df_pairs.to_csv(out / "v20z_tensor_interaction_configs.csv", index=False)
    ...
    df_grid.to_csv(out / "v20z_tensor_hyperparam_grid.csv", index=False)

    melhor_config = df_pairs.loc[df_pairs["holdout_auc_2026"].idxmax()]
    melhor_grid = df_grid.loc[df_grid["holdout_auc_2026"].idxmax()]
    melhor_geral = max(float(melhor_config["holdout_auc_2026"]), float(melhor_grid["holdout_auc_2026"]))
    ...
    resumo = {
        ...
        "melhor_auc_par": round(float(melhor_config["holdout_auc_2026"]), 4),
        "melhor_auc_grid_hiperparametro": round(float(melhor_grid["holdout_auc_2026"]), 4),
    ...
    (out / "v20z_all_reports.json").write_text(
