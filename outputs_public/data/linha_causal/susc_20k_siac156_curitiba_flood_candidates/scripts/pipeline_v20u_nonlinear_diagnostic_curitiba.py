...
RESULTS = next(_p for _p in (Path(__file__).resolve(), *Path(__file__).resolve().parents) if (_p / ".git").is_dir() and (_p / "environment.yml").is_file()) / (
    "outputs_public/data/linha_causal/susc_20k_siac156_curitiba_flood_candidates/results")
...
def gbm_temporal_holdout_with_ci(df_train: pd.DataFrame, df_test: pd.DataFrame,
    ...
    return {"n_train": int(len(dtr)), "n_test": int(len(dte)), "point_auc": round(float(point), 4),
...
def gbm_spatial_block_cv(unidades: pd.DataFrame, feature_cols: list[str],
    ...
    for i, (tr_idx, te_idx) in enumerate(gkf.split(dnd, groups=dnd["bairro"].values)):
        ...
        clf.fit(scaler.transform(Xtr), ytr)
        score = clf.predict_proba(scaler.transform(Xte))[:, 1]
        rows.append({"fold": i, "n_train": int(len(dtr)), "n_test": int(len(dte)),
                     "holdout_auc_gbm": round(float(roc_auc_score(yte, score)), 4)})
...
def main() -> int:
    ...
    d2.to_csv(out / "v20u_gbm_spatial_block_cv.csv", index=False)
    mean_auc = round(float(d2["holdout_auc_gbm"].mean()), 4)
    ...
    d3.to_csv(out / "v20u_gbm_hyperparam_sensitivity.csv", index=False)
    reports["3_hyperparam_sensitivity"] = {
        "auc_min": round(float(d3["holdout_auc"].min()), 4),
        "auc_max": round(float(d3["holdout_auc"].max()), 4),
        "auc_median": round(float(d3["holdout_auc"].median()), 4),
        "frac_configs_above_0_55": round(float((d3["holdout_auc"] > 0.55).mean()), 4),
    }
    print(json.dumps(reports["3_hyperparam_sensitivity"], indent=2))

    (out / "v20u_all_reports.json").write_text(
