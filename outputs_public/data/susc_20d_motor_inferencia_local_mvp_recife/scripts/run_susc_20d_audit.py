...
from susc_20d_score_engine import (
...
def validate_point_estimate(X: np.ndarray, y: np.ndarray, scaler: StandardScaler) -> dict:
    ...
    res = fit_firth(Xd, y)
    published = {r["feature"]: float(r["coef_standardized"]) for r in read_csv(IN_PUBLISHED_COEFS)}
    max_abs_diff = 0.0
...
def validate_bootstrap(X: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    max_abs_diff = 0.0
    for i, feat in enumerate(FEATURE_COLS, start=1):
        pub = next(r for r in published if r["feature"] == feat)
...
def run() -> None:
    ...
    for j in kept:
        ...
        if dino_available:
            try:
                ...
                dino_evidence = None
        rows_out.append({
            "point_id": j["point_id"], "label_real": j["label"],
            "score": out["score"], "ci_low": out["confidence_interval"][0],
            "ci_high": out["confidence_interval"][1],
            "top_feature": out["features_used"][0]["feature"],
            "top_feature_contribution": round(out["features_used"][0]["contribution"], 4),
            "dino_evidence_patch_id": (dino_evidence or {}).get("patch_id", ""),
            "model_version": MODEL_VERSION, "region": REGION_SUPPORTED,
        })
        scores_for_auc.append(out["score"])

    in_sample_auc = float(roc_auc_score(y, scores_for_auc))
    documented_loo_auc = 0.6781
    ci_widths = [r["ci_high"] - r["ci_low"] for r in rows_out]

    RESULTS.mkdir(parents=True, exist_ok=True)
    with (RESULTS / "susc_20d_score_audit_v1.csv").open("w", encoding="utf-8", newline="") as fh:
    ...
    summary = {
        ...
        "model_version": MODEL_VERSION, "region_supported": [REGION_SUPPORTED],
        "limitations": LIMITATIONS,
        "status": "SUSC_20D_MVP_AUDIT_READY_REVIEW_ONLY",
    }
    with (RESULTS / "susc_20d_audit_summary.json").open("w", encoding="utf-8") as fh:
