...
def qa_decision(bairro_reconhecido: bool, rua_generica: bool, tem_logradouro: bool, tem_bairro: bool) -> tuple[str, list[str]]:
    issues = []
...
def run_qa(rows: list[dict], bairros_referencia: set[str]) -> list[dict]:
    ...
    for row, key in zip(rows, keys):
        ...
        decision, issues = qa_decision(bairro_reconhecido, rua_generica, bool(logradouro), bool(bairro))

        qa_id = stable_hash(row.get("source_year", ""), logradouro, bairro, row.get("DataCriacao", ""))
        out.append(
            {
                "qa_record_id": qa_id,
                ...
                "duplicate_group_count": dup_counts[key],
                "qa_decision": decision,
                "qa_issues": ";".join(issues) if issues else "none",
                "allowed_use": "Candidato a geocodificacao; nao e coordenada, label ou evidencia de treino.",
                "prohibited_use": "Nao usar como ground truth, score ou feature derivada do label.",
            }
        )
    return out
...
def main(argv=None) -> int:
    ...
    decisions = Counter(r["qa_decision"] for r in out_rows)
