...
def adjudicate_point(
    ...
    for key, (arr, valid, src) in arrays.items():
        ...
        out.setdefault("pixel_size_m", round(abs(src.transform.a), 4))
        row_col = (row, col)
    ...
    if row_col is not None:
        ...
        if 0 <= row < dist_raster.shape[0] and 0 <= col < dist_raster.shape[1]:
            out["dist_to_extracted_stream_m"] = round(float(dist_raster[row, col]), 1)
        else:
            out["dist_to_extracted_stream_m"] = None
    else:
        out["dist_to_extracted_stream_m"] = None
...
def adjudicate_candidates(geocoded_rows: list[dict], raster_dir: Path | str) -> list[dict]:
    ...
    for row in geocoded_rows:
        if not row.get("lat") or not row.get("lon"):
            merged = dict(row)
            merged.update({"adjudication_status": "SEM_COORDENADA_GEOCODIFICADA"})
        ...
        merged["adjudication_status"] = "LIDO"
        out_rows.append(merged)
    return out_rows
...
def main(argv=None) -> int:
    ...
    n_lido = sum(1 for r in out_rows if r.get("adjudication_status") == "LIDO")
