...
OUT_CSV = REGISTRIES / "v20l_dataset_curitiba_features_v1.csv"
...
# feature -> arquivo raster (elevacao vem do MDT bruto, nao do dem_filled hidro-condicionado,
...
def load_points(pos_csv: Path = POS_CSV, neg_csv: Path = NEG_CSV) -> pd.DataFrame:
    ...
    # (point_id, event_date) sao a MESMA ocorrencia categorizada duas vezes no SIAC 156
    ...
    key = df["lat"].round(7).astype(str) + "|" + df["lon"].round(7).astype(str) + "|" + df["event_date"].dt.strftime("%Y-%m-%d")
    df["observation_unit_key"] = [hashlib.sha1(k.encode()).hexdigest()[:16] for k in key]
    df["is_duplicate_observation_unit"] = df.duplicated("observation_unit_key", keep="first")
    return df
...
def extract_rain(df: pd.DataFrame, cache_dir: Path, batch: int = 100, sleep_s: float = 0.5) -> pd.DataFrame:
    ...
    span_start = df["event_date"].min().date() - timedelta(days=LOOKBACK_DAYS)
    span_end = df["event_date"].max().date() - timedelta(days=1)
    ...
    for r in df.itertuples():
        ...
        if series is None:
            ...
            continue
        f = rain_window_features(series, r.event_date.date())
...
def extract_mapbiomas(df: pd.DataFrame, cache_dir: Path, chunk: int = 400) -> pd.DataFrame:
    ...
    out["landcover_source"] = "mapbiomas_collection9_gee"
...
def main(argv=None) -> int:
    ...
    prov = {
        ...
        "n_observation_units": int(df["observation_unit_key"].nunique()),
        "n_observation_units_positive": int(df.loc[df.label == 1, "observation_unit_key"].nunique()),
        "n_observation_units_negative": int(df.loc[df.label == 0, "observation_unit_key"].nunique()),
        "terrain": {
        ...
        "rain": {
            ...
            "decay_k": DECAY_K,
            "window_rule": "14 dias terminando no dia anterior ao event_date (dia do evento excluido)",
