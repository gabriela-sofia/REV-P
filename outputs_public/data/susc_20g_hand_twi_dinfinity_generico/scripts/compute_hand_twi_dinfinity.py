...
def compute_hand_twi_dinf(
    dtm_path: Path | str,
    outdir: Path | str,
    stream_percentile: float = STREAM_PERCENTILE_V12,
    ...
    with rasterio.open(paths["accum_cells"]) as src:
        accum = src.read(1)
    threshold = float(np.percentile(accum[mask], stream_percentile))
    del accum
    ...
    return DinfResult(
        ...
        valid_min=valid_min,
        wbt_version=str(wbt.version()).splitlines()[0].strip(),
...
def main(argv=None) -> int:
    ...
    p.add_argument("--region-label", default="unlabeled")
