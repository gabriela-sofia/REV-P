...
def detect_water_candidates_sar(
    ...
    vh_available = "VH" in bands_before and "VH" in bands_after
    if vh_available:
    ...
    if len(keep):
        ...
        for i, lab in enumerate(keep):
            ...
            entry = {
                ...
                "vv_after_db_mean": round(float(vv_after_mean[i]), 2),
                "vv_before_db_mean": round(float(vv_before_mean[i]), 2),
                "drop_db_mean": round(float(vv_before_mean[i] - vv_after_mean[i]), 2),
                "vh_available": vh_available,
                "adjudicated": "false",
            }
...
def main(argv=None) -> int:
    ...
    res = detect_water_candidates_sar(before, after, cfg, transform=transform, crs=crs, extra_mask=extra_mask)
