...
def test_ranker_orders_targets_by_score(tmp_path, monkeypatch):
    ...
    # With the v2 schema migrated and valid, locking it via QA harness is rank 1.
    assert rows[0]["next_target"] == "EVENT_PATCH_PACKAGE_V2_QA_HARNESS"
    assert rows[0]["recommended_version"].startswith("v2ad")
