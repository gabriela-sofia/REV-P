...
from tests.test_revp_v2as_common import install_all, read_csv
...
def test_orchestrator_runs_all_steps(tmp_path, monkeypatch):
    ...
    assert rows[0]["step_name"] == "deep_probe_priority"
...
def test_orchestrator_fail_fast(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(common, "run_geometry_payload_detector", boom)
    with pytest.raises(ValueError):
        common.run_master_orchestrator(common.parse_args([]))
    manifest = read_csv(protocol / "v2as_orchestrator_run_manifest.csv")
    assert manifest[-1]["status"] == "FAIL"
    assert manifest[-1]["step_name"] == "geometry_payload_detector"
