"""Tests for v1mk bulk GT final report."""
...
SCRIPT = ROOT / "scripts/protocolo_c/revp_v1mk_bulk_gt_acquisition_final_report.py"
OUT = ROOT / "datasets/protocol_c_bulk_gt_acquisition_summary.csv"
DOCS = [
    ROOT / "docs/metodologia_cientifica/protocolo_c_aquisicao_maciça_ground_truth_v1ma_v1mk.md",
    ROOT / "docs/metodologia_cientifica/protocolo_c_relatorio_aquisicao_maciça_ground_truth_v1ma_v1mk.md",
]
...
def test_summary_keeps_c4_closed_without_formal_negative() -> None:
    row = rows(OUT)[0]
    if row["best_negative_evidence"] == "none":
...
def test_public_outputs_have_no_private_paths_or_raw_refs() -> None:
