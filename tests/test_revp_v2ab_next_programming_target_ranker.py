...
def test_ranker_orders_targets_by_score(tmp_path, monkeypatch):
    ...
    # With the schema contract built and packages needing migration, the
    # implementation step is the top score-based target.
    assert rows[0]["next_target"] == "EVENT_PATCH_SCHEMA_MIGRATION_IMPLEMENTATION"
    assert rows[0]["recommended_version"].startswith("v2ac")
