...
from tests.test_revp_v2ac_event_patch_v2_package_builder import install_base_inputs, read_csv, set_env
...
def test_completion_report_and_manifest(tmp_path, monkeypatch):
    ...
    assert result["schema_valid"] >= 3
    assert result["next_target"]

    next_actions = read_csv(str(data / "v2ac_next_actions_registry.csv"))
    assert next_actions[0]["status"] == "RECOMMENDED_NEXT_STEP"

    manifest = read_csv(str(data / "v2ac_versionable_artifacts_manifest.csv"))
    assert manifest
    assert all(r["is_versionable"] == "true" for r in manifest)
    assert all(r["protocol_version"] == "v2ac" for r in manifest)

    docs = str(tmp_path / "docs" / "metodologia_cientifica")
    status = open(os.path.join(docs, "protocolo_c_status_atual_v2ac.md"), encoding="utf-8").read()
    assert common.MAX_STATUS in status
    report = open(os.path.join(docs, "protocolo_c_relatorio_v2ac_event_patch_schema_migration.md"), encoding="utf-8").read()
...
def test_no_forbidden_true_in_any_v2ac_csv(tmp_path, monkeypatch):
    data = set_env(tmp_path, monkeypatch)
    install_base_inputs(data)
    common.run_all(common.parse_args([]))
    for name in os.listdir(str(data)):
        if not name.startswith("v2ac_") or not name.endswith(".csv"):
            continue
