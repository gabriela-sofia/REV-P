...
from tests.test_revp_v2ab_patch_namespace_inventory import install_base_inputs, read_csv, set_env
...
def test_completion_report_and_manifest(tmp_path, monkeypatch):
    ...
    assert result["namespaces"] >= 4
    assert result["next_target"]

    next_actions = read_csv(str(data / "v2ab_next_actions_registry.csv"))
    assert next_actions[0]["status"] == "RECOMMENDED_NEXT_STEP"

    manifest = read_csv(str(data / "v2ab_versionable_artifacts_manifest.csv"))
    assert manifest
    assert all(r["is_versionable"] == "true" for r in manifest)
    assert all(r["protocol_version"] == "v2ab" for r in manifest)

    docs = str(tmp_path / "docs" / "metodologia_cientifica")
    status = open(os.path.join(docs, "protocolo_c_status_atual_v2ab.md"), encoding="utf-8").read()
    assert common.MAX_STATUS in status
    report = open(os.path.join(docs, "protocolo_c_relatorio_v2ab_event_patch_package_schema_hardening.md"), encoding="utf-8").read()
...
def test_no_forbidden_true_in_any_v2ab_csv(tmp_path, monkeypatch):
    data, scan = set_env(tmp_path, monkeypatch)
    install_base_inputs(data, scan)
    common.run_all(common.parse_args([]))
    for name in os.listdir(str(data)):
        if not name.startswith("v2ab_") or not name.endswith(".csv"):
            continue
